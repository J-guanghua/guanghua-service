...
use Yii;
class WxCustomer extends \yii\mongodb\ActiveRecord//\yii\db\ActiveRecord
{
    ...
    {
        return [
            [['fromusername','appid','avatar'], 'required'],
            [['fromusername'], 'unique'],
            ...
            [['sign','appid','username','fromusername','status'], 'string', 'max' => 32],
        ];
    }
    public function transform()
    {
        return ['id','created_at','updated_at','identity','fd','genre'];
    }
     /**
     * @inheritdoc
     */
    ...
    {
        return ['_id','id', 'username','fromusername','sign', 'status','created_at','updated_at','identity','avatar','fd','appid','genre'];
    }
    /**
     * @inheritdoc
     */
    ...
    {   
        ...
        if (null !== $relevance = WxRelevance::findOne(['fo_id'=>$fo_id,'status'=>1])) {
            if(null === $service = $model->andWhere(['and',['id'=>$relevance->to_id],['!=','status',WxCustomer::STATUS_OFFLINE]])->One()){
    ...
    public function sendWechatMsg($to_id,array $content)
    {
        ...
        if (($result['errcode']) && $result['errcode'] != 0) {
            $result['errmsg'] = isset(WxMessage::$errors[$result['errcode']]) ? WxMessage::$errors[$result['errcode']] : '错误状态码:'.$result['errcode'];
    ...
    {   
        $to_id = (int) $to_id;
        if ((null !== $model = WxRelevance::findOne(['fo_id'=>$this->id,'status'=>WxCustomer::IDENTITY_ONE])) && $model->to_id != $to_id) {
