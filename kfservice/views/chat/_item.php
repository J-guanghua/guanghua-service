...
#className{  position: relative;
    ...
    width: 100%;
    height: 630px;};
.layim-chat-mine .layim-chat-text{width: 50px}
</style>
...
<div class="modal-dialog modal-lg"  id="message" style="display: none;z-index: 1000044444444">
  <div class="modal-content">
    <div class="modal-header">
    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
    <span style="color: #e26005;font-size: 18px;">小程序配置</span>
    </div>
    <div class="modal-body" id="className">
...
/*
 ...
 // layim.setChatMin();
  ...
  layim.on('tool(code)', function(insert, send, obj){ //事件中的tool为固定字符，而code则为过滤器，对应的是工具别名（alias）
    ...
    // }, function(text, index){
    //   layer.close(index);
    //   text = parent.layui.layim.content(text)
    ...
    //   fixed: false, //不固定
    //   maxmin: true,
    //   content: "<?=Url::to(['index'])?>"
    // });
    // $.get("<?=Url::to(['modal'])?>", function(data){
    //   //var user = JSON.parse(data);
    //     console.log(data); 
    //     $(".modal-body").html(data) 

    // });
    // if($('#className').html().length > 0){
    //   $("#message").modal();
    // } else {  
    if(go_){
      go_=false;
    $('#className').one("dblclick",".wechatjson",function(){ 
        console.log(5555);
      wechatmag = $(this).data('wechat');
      $("#message").modal('hide');
      insert('[pre class=layui-code]<div class="thumbnail"><img src="/static/image/20171221/1513836229428222.png" alt="..." style="width:100%;height:100px"><div class="caption"><p>骆驼官方优品商城</p><div class="row"><div class="col-md-6" style="text-align: center;"><a href="/admin/index.php?r=app-wechat%2Fupdate&amp;id=3">修改</a></div><div class="col-md-6" style="text-align: center;"><a href="/admin/index.php?r=app-wechat%2Fview&amp;id=3">详情</a></div></div></div></div>[/pre]');
        // console.log(send);
        send();
      go_=true;
  ...
  //文本内容....<a href='http://www.qq.com' style='color:red' data-miniprogram-appid='wxf8ef6247c7de6285' data-miniprogram-path='pages/index/index'>点击跳小程序</a>
  //监听layim建立就绪
...
//切换窗口得到的信息
  ...
  layim.on('chatChange', function (res) {
   ...
   //   console.log("sdas:" + JSON.stringify(res));
    ...
    $("#tips-"+res.data.id).removeAttr("style");
    $("#tips-"+res.data.id).data('number',0)
     $("#layui-layim-chat .layim-friend"+res.data.id).find('.layim-msg-status').remove()
   // console.log(res.data.id)
    if(type === 'friend'){
      //模拟标注好友状态
        //layim.setChatStatus('<span style="color:#FF5722;">在线</span>');
    ...
    //首先连接websocket
     var socket = new WebSocket('ws://172.18.42.200:9053');
      ...
      //alert()  
        ...
        // }));
        $.get("<?=Url::to(['message-user'])?>", function(data){
          //var user = JSON.parse(data);
            //console.log(data.id);  
            socket.send(JSON.stringify({
                action: 'login',//随便定义，用于在服务端区分消息类型
      ...
      socket.onmessage = function (res) {
          ...
          if (message.action === 'chatMessage') {
            var number = $("#tips-"+message.data.id).data('number')
            if(number == undefined){
              $("#tips-"+message.data.id).data("number", 1);
            }else{
              $("#tips-"+message.data.id).data("number", number+1);
            }
            $("#tips-"+message.data.id).text($("#tips-"+message.data.id).data('number'))
            $("#tips-"+message.data.id).show()
            if($("#layui-layim-chat .layim-friend"+message.data.id).find('.layim-msg-status').length > 0){
              
              $("#layui-layim-chat .layim-friend"+message.data.id).find('.layim-msg-status').text($("#tips-"+message.data.id).data('number'));
            } else {
              $("#layui-layim-chat .layim-friend"+message.data.id).append($("#tips-"+message.data.id).clone().css({"display":"inline","position": "relative","right": "-10px","bottom": "-12px","padding": "0 5px","height": "16px","line-height": "16px","border-radius": "8px","text-align": "center","font-size": "10px","background-color": "#dd4b39","color": "#fff","z-index":"-2","font-family": "monospace"}).attr('id',''))
            }
            // $(".layui-unselect .layim-friend"+message.data.id).append('<div class="message-number">3</div>')
            //$(".layui-unselect .layim-friend"+message.data.id).data('message-number',3)
