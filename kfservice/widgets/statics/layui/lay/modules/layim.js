...
function (i) {
    ...
    c.prototype.getPolling = function(i){
        for (var n = [], l = j.friend || [], o = "", d = 0; d < l.length; d++){
          for (var r = 0; r < (l[d].list || []).length; r++){
            if(l[d].list[r].id == i.id && l[d].list[r].unread > 0){
                e("#layui-layim-chat .layim-friend"+i.id).find('.layim-msg-status').remove()
                e("#layui-layim-chat .layim-friend"+i.id).append('<span class="layim-msg-status" style="display:inline;position:relative;right:120px;bottom:5px;padding:0 5px;height:16px;line-height:16px;border-radius:8px;text-align:center;font-size:10px;background-color:#dd4b39;color:#fff;z-index:1;font-family:monospace">'+l[d].list[r].unread+'</span>');
            } 
          } 
        } 
    },
    c.prototype.setPolling = function(i,index){
        var index = (0 | index);
        var es = x.find("#layui-layim-search-"+index)
        if (i != undefined && i.isnew) {
            console.log(i.timestamp)
            this.addList(i);
        }
        for (var n = [], l = j.friend || [], o = "", d = 0; d < l.length; d++){
          for (var r = 0; r < (l[d].list || []).length; r++){
            if(i != undefined && l[d].list[r].id == i.id){
              j.friend[d].list[r].unread = i.unread
              j.friend[d].list[r].last_msg = i.last_msg
              j.friend[d].list[r].timestamp = i.timestamp
              i.unread == 1 ? j.friend[d].list[r].for_time = i.timestamp*1000 : null
            } 
            (l[d].list[r].type = "friend", l[d].list[r].index = d, l[d].list[r].list = r, n.push(l[d].list[r]));
            if(l[d].list[r].unread > 0){
                e("#layui-layim-chat .layim-friend"+l[d].list[r].id).find('.layim-msg-status').remove()
                e("#layui-layim-chat .layim-friend"+l[d].list[r].id).append('<span class="layim-msg-status" style="display:inline;position:relative;right:120px;bottom:5px;padding:0 5px;height:16px;line-height:16px;border-radius:8px;text-align:center;font-size:10px;background-color:#dd4b39;color:#fff;z-index:1;font-family:monospace">'+l[d].list[r].unread+'</span>');
            }
          } 
        }
        n =  this.layimSortArr(n);
        console.log(n)
        if (n.length > 0) for (var u = 0; u < n.length; u++) o += '<li layim-event="chat" data-type="' + n[u].type + '" data-index="' + n[u].index + '"  data-list="' + n[u].list + '"><img src="' + n[u].avatar + '"><span>' + (n[u].username || n[u].groupname || "佚名") + "</span><cite><i style='position:absolute;right:25px;font-size:3px;color:#ccc'>"+layui.data.dateday(n[u].timestamp*1000)+"</i></cite><p id='message-"+n[u].id+"'>" + (n[u].last_msg || n[u].remark || n[u].sign || "") + "</p><span id='tips-"+n[u].id+"' class='layim-msg-status' data-timestamp="+n[u].for_time+" data-unread="+n[u].unread+" style='background-color:#ccc;display:"+(n[u].unread?'block':'none')+"'></span></li>";
    ...
    c.prototype.layimSortArr = function (arrays) {
        
        for(all=0;all<arrays.length-1;all++){
            for(one=0;one<arrays.length-1;one++){
                if(arrays[one]['for_time']<arrays[one+1]['for_time']){
                    var temp=arrays[one];
                    arrays[one]=arrays[one+1];
                    arrays[one+1]=temp;
                }
            }
        }
    ...
    c.prototype.setInterval = function(){
        var index = (0 | 0),poor = 0;
        setInterval(function(){
            var number = 0;
	    if(x==undefined) return false;
            x.find("#layui-layim-search-0>li").each(function(key,value){
                value = e(value).find('.layim-msg-status')
                if(value.data('unread') > 0){
                    number++;
                    value.show()
                    var timestamp = Date.parse(new Date())+poor
                    var s = (timestamp-value.data('timestamp'))/1000
                    if(s < 0 && poor==0){
                        poor = -(s)*1000,s = 0;
                    }
                    var h = Math.floor(s/60)+'m';
                    if(h=='0m'){
                        h = (s-Math.floor(s/60)*60)+'s'
                    }
                    if(Math.floor(s/60) >= 60){
                        h = Math.floor(s/60/60)+'h'
                    }
                    value.css('background-color','#dd4b39')
                    value.html(h)
                }  
             });
            if(number){
                x.find(".layui-layim-tab>li").eq(index).find('.layui-badge').remove()
                x.find(".layui-layim-tab>li").eq(index).append('<span class="layui-badge" style="background-color:#dd4b39;color:#fff;font-size:10px;height:16px;display:block;width:26px;position:relative;top:-30px;right:-40px;border-radius:6px;line-height:16px;text-align:center">'+number+'</span>');
            }else{
                x.find(".layui-layim-tab>li").eq(index).find('.layui-badge').remove()
            }
            var num = x.find("#layui-layim .layim-list-group > li").children('img').length
            if(num){
                x.find(".layui-layim-tab>li").eq(1).find('.layui-badge').remove()
                x.find(".layui-layim-tab>li").eq(1).append('<span class="layui-badge" style="background-color:#dd4b39;color:#fff;font-size:10px;height:16px;display:block;width:26px;position:relative;top:-30px;right:-40px;border-radius:6px;line-height:16px;text-align:center">'+num+'</span>');
            }else{
                x.find(".layui-layim-tab>li").eq(1).find('.layui-badge').remove()
            }
        },1000);
    ...
    y = ['<div class="layui-layim-main">', '<div class="layui-layim-info">', '<div class="layui-layim-user">{{ d.mine.username }}</div>', '<div class="layui-layim-status">', '{{# if(d.mine.status === "online"){ }}', '<span class="layui-icon layim-status-online" layim-event="status" lay-type="show">&#xe617;</span>', '{{# } else if(d.mine.status === "hide") { }}', '<span class="layui-icon layim-status-hide" layim-event="status" lay-type="show">&#xe60f;</span>', "{{# } }}", '<ul class="layui-anim layim-menu-box">', '<li {{d.mine.status === "online" ? "class=layim-this" : ""}} layim-event="status" lay-type="online"><i class="layui-icon">&#xe618;</i><cite class="layui-icon layim-status-online">&#xe617;</cite>在线</li>', '<li {{d.mine.status === "hide" ? "class=layim-this" : ""}} layim-event="status" lay-type="hide"><i class="layui-icon">&#xe618;</i><cite class="layui-icon layim-status-hide">&#xe60f;</cite>挂起</li>', "</ul>", "</div>", '<input class="layui-layim-remark" placeholder="编辑签名" value="{{ d.mine.remark||d.mine.sign||"" }}">', "</div>", '<ul class="layui-unselect layui-layim-tab{{# if(!d.base.isfriend || !d.base.isgroup){ }}', " layim-tab-two", '{{# } }}">', '<li class="layui-icon', "{{# if(!d.base.isfriend){ }}", " layim-hide", "{{# } else { }}", " layim-this", "{{# } }}", '" title="联系人" layim-event="tab" lay-type="friend">&#xe612;</li>', '<li class="layui-icon', "{{# if(!d.base.isgroup){ }}", " layim-hide", "{{# } else if(!d.base.isfriend) { }}", " layim-this", "{{# } }}", '" title="待接入会话" layim-event="tab" lay-type="group">&#xe613;</li>', '<li class="layui-icon" title="历史会话" layim-event="tab" lay-type="history">&#xe611;</li>', '<li class="layui-icon" title="公众号" layim-event="tab" lay-type="wechat">&#xe611;</li>', "</ul>", '<ul class="layui-unselect layim-tab-content {{# if(d.base.isfriend){ }}layui-show{{# } }} layim-list-friend">', '{{# layui.each(d.friend, function(index, item){ var spread = d.local["spread"+index]; }}', "<li>", '<h5 layim-event="spread" lay-type="{{ spread }}"><i class="layui-icon">{{# if(spread === "true"){ }}&#xe61a;{{# } else {  }}&#xe602;{{# } }}</i><span>{{ item.groupname||"未命名分组"+index }}</span><em>(<cite class="layim-count"> {{ (item.list||[]).length }}</cite>)</em></h5>', '<ul id="layui-layim-search-0" class="layui-layim-list {{# if(spread === "true"){ }}', " layui-show", '{{# } }}">', u({
        ...
        type: "group"
    }), "</ul>", "</li>", "</ul>", '<ul class="layui-unselect layim-tab-content  {{# if(!d.base.isfriend && !d.base.isgroup){ }}layui-show{{# } }}">', "<li>", '<ul id="layui-layim-search-2" class="layui-layim-list layui-show layim-list-history">', u({
        type: "history"
    }), "</ul>", "</li>", "</ul>", '<ul class="layui-unselect layim-tab-content  {{# if(!d.base.isfriend && !d.base.isgroup){ }}layui-show{{# } }}">', "<li>", '<ul class="layui-layim-list layui-show">', u({
        type: "wechat"
    }), "</ul>", "</li>", "</ul>", '<ul class="layui-unselect layim-tab-content">', "<li>", '<ul class="layui-layim-list layui-show"></ul>', "</li>", "</ul>", '<ul class="layui-unselect layui-layim-tool">', '<li class="layui-icon layim-tool-search" layim-event="search" title="搜索">&#xe615;</li>', "{{# if(d.base.msgbox){ }}", '<li class="layui-icon layim-tool-msgbox" layim-event="msgbox" title="消息盒子">&#xe645;<span class="layui-anim"></span></li>', "{{# } }}", "{{# if(d.base.find){ }}", '<li class="layui-icon layim-tool-find" layim-event="find" title="查找">&#xe608;</li>', "{{# } }}", '<li class="layui-icon layim-tool-skin" layim-event="skin" title="更换背景">&#xe61b;</li>', "{{# if(!d.base.copyright){ }}", '<li class="layui-icon layim-tool-about" layim-event="about" title="关于">&#xe60b;</li>', "{{# } }}", "</ul>", '<div class="layui-layim-search"><input><label class="layui-icon" layim-event="closeSearch">&#x1007;</label></div>', "</div>"].join(""),
    ...
    p = ['<div class="layim-add-box">', '<div class="layim-add-img"><img class="layui-circle" src="{{ d.data.avatar }}"><p>{{ d.data.name||"" }}</p></div>', '<div class="layim-add-remark">', '{{# if(d.data.type === "friend" && d.type === "setGroup"){ }}', "<p>选择客服</p>", '{{# } if(d.data.type === "friend"){ }}', '<select class="layui-select" id="LAY_layimGroup">', "{{# layui.each(d.data.group, function(index, item){ }}", '<option value="{{ item.id }}">{{ item.groupname }}</option>', "{{# }); }}", "</select>", "{{# } }}", '{{# if(d.data.type === "group"){ }}', "<p>请输入验证信息</p>", '{{# } if(d.type !== "setGroup"){ }}', '<textarea id="LAY_layimRemark" placeholder="验证信息" class="layui-textarea"></textarea>', "{{# } }}", "</div>", "</div>"].join(""),
    ...
    layui.data.content = function (i) {
        var a = function (i) {
            return new RegExp("\\n*\\[" + (i || "") + "(pre|div|span|p|table|thead|th|tbody|tr|td|ul|li|ol|li|dl|dt|dd|h2|h3|h4|h5)([\\s\\S]*?)\\]\\n*", "g")
                ...
                for(var kk in emoji){
            i = i.replace(emoji[kk].code, function(i){
                return '<img alt="'+emoji[kk].asccode+'" title="'+emoji[kk].alt+'" src="'+X[emoji[kk].asccode]+'">';
            }).replace(' '+emoji[kk].asccode+' ', function(i){
                return '<img alt="'+emoji[kk].asccode+'" title="'+emoji[kk].alt+'" src="'+X[emoji[kk].asccode]+'">';
            });
        }
        return i = (i || "").replace(/err\[([^\*]+?)\]/g,
        function (i) {
            var a = i.replace(/^err/g, "");
            return '<img alt="'+a+'" title="('+a+')" src="'+layui.cache.dir + 'images/imgs/err.png">';
        }).replace(/miniprogrampage\[([^\*\[\]]+?)\]/g,
        function (i) {
            var wechat = JSON.parse(i.replace(/(^miniprogrampage\[)|(\]$)/g, ""));
            var href = wechat.pagepath.indexOf('=')!=-1?(layui.layim.shopHost==undefined?j.base.shopHost:layui.layim.shopHost)+wechat.pagepath.substring(wechat.pagepath.indexOf('=')+1):'javascript:void(0)';
            return '<div class="layui-unselect layui-layim-audio" style="width:260px;height:350px"><p class="layui-icon"><a href="'+href+'" target="_blank">小程序卡片</a></p><p>'+wechat.title+'</p><img class="layui-layim-photos" src="' + wechat.thumburl + '" style="width:240px;height:260px" /></div>'
        }).replace(/link\[([^\*\[\]]+?)\]/g,
        function (i) {
            var wechat = JSON.parse(i.replace(/(^link\[)|(\]$)/g, ""));
            return '<div class="layui-unselect layui-layim-audio" style="width:260px;height:auto"><p class="layui-icon" style="font-size:18px"><a href="'+wechat.url+'" target="_blank">'+wechat.title+'</a></p><p>'+wechat.description+'</p></div>'
        }).replace(/img\[([^\s]+?)\]/g,
        function (i) {
    ...
    L = function (i) {
        return t.open({
            type: 1,
            area: [j.base.width+'px', j.base.height+'px'],
            skin: "layui-box layui-layim",
            title: "&#8203;",
            offset: (j.base.pattern?["rb","auto"]:"rb"),
            id: "layui-layim",
            shade: !1,
            anim: 2,
            resize: !1,
            move:(j.base.pattern?false:'.layui-layer-title'),
            content: i,
            success: function (i) {
                j.base.pattern ? ti.chatLog.wicket = i : null;
                x = i,
                R(i),
    ...
    T = function (i) {
        ...
        t.open({
            ...
            offset: "rb",
            resize: !1,
            content: '<img src="' + (j.mine.avatar || layui.cache.dir + "css/pc/layim/skin/logo.jpg") + '"><span>' + (i || j.base.title || "我的WebIM") + "</span>",
    ...
    z = function (i) {
        ...
        if (a[0]) {
            ...
            1 !== s.find("li").length || o[0] || (d || w.css("width", 750), s.css({
                height: w.height()
        ...
        var u = C = t.open({
            ...
            shade: !1,
            maxmin: !0,
            resize: !(j.base.pattern),
            offset: ['rb', '278px'],
            anim: i.anim || 0,
            closeBtn: !j.base.brief && 1,
            content: n('<ul class="layui-unselect layim-chat-list">' + v + '</ul><div class="layim-chat-box">' + f + "</div>").render(l),
            success: function (a) {
                j.base.pattern ? a.offset({top:ti.chatLog.wicket.offset().top,left:ti.chatLog.wicket.offset().left+j.base.width}) : null;
                w = a,
                a.css({
                    "min-width": "500px",
                    "min-height": "420px"
            ...
            moveEnd: function(i){
                if(j.base.pattern){
                    i.offset({left:i.offset().left<=j.base.width?j.base.width:i.offset().left})
                    ti.chatLog.wicket.offset({top:i.offset().top,left:i.offset().left-j.base.width})
                }
            },
            full: function (i) {
                t.style(u, {
    ...
    $ = function () {
        ...
        if (i.content = a.textarea.val(), "" !== i.content.replace(/\s/g, "")) {
            ...
            if(j.base.linkStatus!==undefined){
                return   t.msg("服务器出错,暂时无法发送消息！");
            }
    ...
    W = function (i) {
        ...
        function (e, t) {
            ...
            j.group.splice(e, 1),
            0 === j.group.length && a.html('<li class="layim-null">暂无会话用户</li>'),
            !0
        }))
    },
    Z = function () {
    ...
    X = function () {
        ...
        var i = [];
        for(var xx in emoji){
            i.push(emoji[xx].asccode)
        }
        a = {};
        return layui.each(i,
        function (i, e) {
            a[e] = layui.cache.dir + "images/imgs/em_" + i + ".png";
        }),
        a;
        //兴财更改（结束）
    }(),
    ...
    ti = {
        ...
        tab: function (i) {
            ...
            //把2更改成3，因为有3个tab
            a > 3 ? t.removeClass(o) : (ti.tab.index = a, i.addClass(o).siblings().removeClass(o)),
        ...
        search: function (i) {
            var index = (0 | ti.tab.index);
            var a = x.find(".layui-layim-search"),
            e = x.find("#layui-layim-search-"+index),
            t = a.find("input"),
            n = function (i) {
                ...
                if (index===2) {
                    var n = []
                    var o = ''
                    layui.each(j.history, function( key, val ) {
                        val.username.indexOf(a) !== -1 && (n.push(val))
                    } );
                    if (n.length > 0) for (var u = 0; u < n.length; u++)
                    o += '<li layim-event="chat" data-type="history" data-index="friend'+n[u].id+'" class="layim-friend'+n[u].id+'"><img src="' + n[u].avatar + '"><span>' + (n[u].username || n[u].groupname || "佚名") + "</span><p>" + (n[u].remark || n[u].sign || "") + "</p><span id='tips-"+n[u].id+"' class='layim-msg-status'>undefined</span></li>";
                    else o = '<li class="layim-null">无搜索结果</li>';
                }else{
                ...
                // else {
                    ...
                    if (n.length > 0) for (var u = 0; u < n.length; u++) o += '<li layim-event="chat" data-type="' + n[u].type + '" data-index="' + n[u].index + '" data-list="' + n[u].list + '"><img src="' + n[u].avatar + '"><span>' + (n[u].username || n[u].groupname || "佚名") + "</span><p>" + (n[u].remark || n[u].sign || "") + "</p><span id='tips-"+n[u].id+"' class='layim-msg-status' style='display:"+(n[u].unread?'block':'none')+"'>"+n[u].unread+"</span></li>";
                    else o = '<li class="layim-null">无搜索结果</li>';
                //}
                }
                e.html(o),
        ...
        closeSearch: function (i) {
            var index = (0 | ti.tab.index);
            var e = x.find("#layui-layim-search-"+index)
            ...
            if (index===2) {
                ...
                for (var n = [], l = j.friend || [], s = j.group || [], o = "", d = 0; d < l.length; d++) for (var r = 0; r < (l[d].list || []).length; r++)  (l[d].list[r].type = "friend", l[d].list[r].index = d, l[d].list[r].list = r, n.push(l[d].list[r]));
                for (var c = 0; c < s.length; c++) (s[c].type = "group", s[c].index = c, s[c].list = c, n.push(s[c]));
                if (n.length > 0) for (var u = 0; u < n.length; u++) o += '<li layim-event="chat" data-type="' + n[u].type + '" data-index="' + n[u].index + '" data-list="' + n[u].list + '"><img src="' + n[u].avatar + '"><span>' + (n[u].username || n[u].groupname || "佚名") + "</span><p>" + (n[u].remark || n[u].sign || "") + "</p><span id='tips-"+n[u].id+"' class='layim-msg-status' style='display:"+(n[u].unread?'block':'none')+"'>"+n[u].unread+"</span></li>";
                else o = '<li class="layim-null">无搜索结果</li>';
            }
            e.html(o)
        ...
        about: function () {
            layer.confirm('清除所有本地消息记录,消息可以在消息记录查看', {
              btn: ['确定','取消'] //按钮
            }, function(){
                localStorage.clear();
        ...
        chatLog: function (i) {
            var a = _();
            var url = j.base.chatLog.indexOf('?')==-1?j.base.chatLog + "?id=" + a.data.id + "&type=" + a.data.type : j.base.chatLog + "&id=" + a.data.id + "&type=" + a.data.type
            if(i.data('url')!= undefined){
                url = i.data('url')
            }
